...
```{r, eval = FALSE}
#download.file(url = "http://www.bom.gov.au/climate/mjo/graphics/rmm.74toRealtime.txt" ,
#              destfile = "DailyMJO.csv", mode = "wb")
...
for (i in 1:dim(BigDays.sfdfT)[1]) {
  ...
  row = MJO_daily %>%
    filter(Date == BigDays.sfdfT$cDate[i])
    MJO_dailyvals <- c(MJO_dailyvals, row$phase)
}
...
for (i in 1:dim(BigDays.sfdfT)[1]) {
  ...
  MJO_5day <- c(MJO_5day, mean(dat$StandardizedMJO))
} 
...
for (i in 1:dim(BigDays.sfdfT)[1]) {
  ...
  startrow = rownum-10
  dat <- MJO_daily[startrow:rownum,]
  MJO_10day <- c(MJO_10day, mean(dat$StandardizedMJO))
} 
...
for (i in 1:dim(BigDays.sfdfT)[1]) {
  ...
  startrow = rownum-20
  dat <- MJO_daily[startrow:rownum,]
  MJO_20day <- c(MJO_20day, mean(dat$StandardizedMJO))
} 
...
for (i in 1:dim(BigDays.sfdfT)[1]) {
  ...
  startrow = rownum-30
  dat <- MJO_daily[startrow:rownum,]
  MJO_30day <- c(MJO_30day, mean(dat$StandardizedMJO))
} 
