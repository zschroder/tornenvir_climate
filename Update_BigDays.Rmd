...
Filter the column names in the `Torn50_19.sfdf` and `Torn2020.sfdf`. 
```{r}
...
```{r}
Jan94 <- crop(January1994, extent(BigDays.sfdfT))
Jan94_extent<- mask(Jan94, BigDays.sfdfT)
plot(Jan94_extent)
```
...
At least you could plot the data. 
```{r}
Jan1994.sfdf <- as.data.frame(January1994, row.names=NULL, optional=TRUE, xy=TRUE, 
             na.rm=TRUE, long=TRUE)
sp::coordinates(Jan1994.sfdf) <- ~ x + y
Help <- st_as_sf(Jan1994.sfdf)

st_crs(Help) <- "+proj=utm +zone=15 +datum=NAD83 +units=m +no_defs" 
...
Jan1994.sfdf <- st_as_sf(as.data.frame(Jan94_extent, row.names=NULL, optional=FALSE, xy=TRUE, na.rm=TRUE, long=TRUE)) #-- > must cut it to the extent of the US
#colnames(Jan1994.sfdf) <- c("Longitude", "Latitude", "Date", "Anomaly")
...
test<- rep(1369:1688, 12)

```{r, eval = FALSE}
jan1994 <- subset(tmp_raster, 1369)
feb94 <- subset(tmp_raster, 1370)
