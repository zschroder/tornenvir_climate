...
```{r}
DLBS_m1 <- lm(maxBS_deep ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day, data = BigDays.sfdfT)
summary(DLBS_m1)
```
...
```{r}
DLBS_m2 <- lmer(maxBS_deep ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m2)
ranef(DLBS_m2) #There is a strong seasonal variation in DLBS values. 
#Changing the non significant to 5, 20, or 30 day data does not change the significance results. 
```

```{r}
DLBS_m3 <- lmer(maxBS_deep ~ SSTgradient_10day + NAO_10day + PNA_10day+ (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m3)
ranef(DLBS_m3) 
```
...
```{r}
plot(residuals(DLBS_m3)) 
```
There is no relationship between the residuals.

*Make a plot of the standardized residuals for the DLBS model*
```{r}
DLBSmod.df <- fortify.merMod(model=DLBS_m3, data = getData(DLBS_m3))
DLBSmod.df$predDLBS <- predict(DLBS_m3)
...
```{r}
CAPE_env <- data.frame(SSTgradient = BigDays.sfdfT$SSTgradient, ENSO = BigDays.sfdfT$avg_Nino3410day, NAO = BigDays.sfdfT$NAO_10day, PNA = BigDays.sfdfT$PNA_10day, MJO = BigDays.sfdfT$MJO_10day, CAPE = BigDays.sfdfT$maxCAPE)
...
```{r}
CAPE_m1 <- lm(maxCAPE ~  SSTgradient_20day + avg_Nino3410day + NAO_30day + PNA_5day +  MJO_10day , data = BigDays.sfdfT)
summary(CAPE_m1)
...
```{r}
CAPE_m2 <- lmer(maxCAPE ~ SSTgradient_20day + avg_Nino3410day + NAO_30day + PNA_5day +  MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(CAPE_m2)
ranef(CAPE_m2) #There is a strong seasonal variation in CAPE values. 
```

```{r}
CAPE_m3 <- lmer(maxCAPE ~  SSTgradient_20day + PNA_5day + MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(CAPE_m3)
ranef(CAPE_m3) #There is a strong annual variation in CAPE values  
```

```{r}
CAPE_m4<- lmer(maxCAPE ~ PNA_5day + MJO_10day + (1|Mo) , data = BigDays.sfdfT)
summary(CAPE_m4)
ranef(CAPE_m4)
```
...
```{r}
plot(residuals(CAPE_m3)) 
```
There is no relationship between the residuals.

*Make a plot of the standardized residuals for the CAPE model*
```{r}
CAPEmod.df <- fortify.merMod(model=CAPE_m3, data = getData(CAPE_m3), na.rm = TRUE)
CAPEmod.df$predCAPE <- predict(CAPE_m3)
