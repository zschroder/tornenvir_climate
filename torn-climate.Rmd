...
p1 <- tm_shape(CAPE_extent) +
  ...
  tm_format(title = expression(paste("CAPE [J / kg ]")), "World", legend.position = c("right", "bottom"),
                   attr.position = c("right", "top"),
                   legend.frame = FALSE,
...
p4 <- tm_shape(BS_extent) +
  ...
  tm_format(title = expression(paste("Deep-Layer Bulk Shear [m / s]")), "World", legend.position = c("right", "bottom"),
...
( p0 <- ggplot(DLBSByMonth, aes(x = Mo, y = avgDLBS)) +
 ...
 scale_x_continuous(breaks = seq(1, 12, 1), labels = month.abb) +
  ...
  coord_polar(start =0) +
  labs(x = "Month", y = "Normalized Averages of DLBS [m/s]") +
  #ggtitle("a") +
  theme_minimal() )
```
...
( p1 <- ggplot(DLBSByHour, aes(x = as.factor(Hour_UTC), y = avgDLBS)) +
  ...
  labs(x = "Hour (UTC)", y = "Average Extremes of DLBS [m/s]") +
  theme_minimal() )
```
...
Models: 
```{r}
DLBS_m2 <- lmer(maxBS_deep ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m2)
ranef(DLBS_m2) #There is a strong seasonal variation in DLBS values. 
```

```{r}
DLBSmod_coefs <- summary(DLBS_m2)
...
```{r}
DLBSmod.df <- fortify.merMod(model=DLBS_m8, data = nlme::getData(DLBS_m8))
...
( p2 <- ggplot(SLBSByMonth, aes(x = Mo, y = avgSLBS)) +
 ...
 scale_x_continuous(breaks = seq(1, 12, 1), labels = month.abb) +
  ...
  coord_polar(start =0) +
  labs(x = "Month", y = "Normalized Averages of SLBS [m/s]") +
      #ggtitle("b") +
  theme_minimal() )
```
...
( p3 <- ggplot(SLBSByHour, aes(x = as.factor(Hour_UTC), y = avgSLBS)) +
  ...
  coord_polar(start =-0.2) +
  labs(x = "Hour (UTC)", y = "Average Extremes of SLBS [m/s]") +
  theme_minimal() )
```
...
Initial Model: 
```{r}
SLBS_m1 <- lm(maxBS_shallow ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + Lat + Lon, data = BigDays.sfdfT)
summary(DLBS_m1)
```

Models: 
```{r}
SLBS_m2 <- lmer(maxBS_shallow ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(SLBS_m2)
ranef(SLBS_m2) #There is a strong seasonal variation in DLBS values. 
```

```{r}
SLBSmod_coefs <- summary(SLBS_m2)
xtable(SLBSmod_coefs$coefficients, digits = 3)
```


```{r}
SLBS_m3 <- lmer(maxBS_shallow ~ SSTgradient_10day + PNA_10day +  MJO_10day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(SLBS_m3)
ranef(SLBS_m3) 
```

```{r}
SLBS_m4 <- lmer(maxBS_shallow ~ SSTgradient_20day + avg_Nino3420day + NAO_20day + PNA_20day +  MJO_20day + Lat + Lon +(1|Mo), data = BigDays.sfdfT)
summary(SLBS_m4)
ranef(SLBS_m4)
```

```{r}
SLBS_m5 <- lmer(maxBS_shallow ~ SSTgradient_30day + avg_Nino3430day + NAO_30day + PNA_30day +  MJO_30day + Lat + Lon +(1|Mo), data = BigDays.sfdfT)
summary(SLBS_m5)
ranef(SLBS_m5)
```

```{r}
SLBS_m6 <- lmer(maxBS_shallow ~ SSTgradient_5day + avg_Nino345day + NAO_5day + PNA_5day +  MJO_5day + Lat + Lon +(1|Mo), data = BigDays.sfdfT)
summary(SLBS_m6)
ranef(SLBS_m6)
```

```{r}
SLBS_m7 <- lmer(maxBS_shallow ~ SSTgradient_20day + avg_Nino3430day + NAO_20day + PNA_10day +  MJO_30day + Lat + Lon +(1|Mo), data = BigDays.sfdfT)
summary(SLBS_m7)
ranef(SLBS_m7)
```

```{r}
SLBS_m8 <- lmer(maxBS_shallow ~ SSTgradient_20day + NAO_20day + PNA_10day +  MJO_30day + Lat + Lon + Year + (1|Mo), data = BigDays.sfdfT)
summary(SLBS_m8)
ranef(SLBS_m8)
```
...
*Model 8 is the best*
```{r}
AIC(SLBS_m1, SLBS_m2, SLBS_m3, SLBS_m4, SLBS_m5, SLBS_m6, SLBS_m7, SLBS_m8)
```

*Check the residuals of model 8 of SLBS.*
```{r}
plot(residuals(SLBS_m8)) 
```
There is no relationship between the residuals.

*Make a plot of the standardized residuals for the SLBS model*
```{r}
SLBSmod.df <- fortify.merMod(model=SLBS_m8, data = nlme::getData(SLBS_m8))
SLBSmod.df$predSLBS <- predict(SLBS_m8)
...
( p4 <- ggplot(CAPEByMonth, aes(x = as.factor(Mo), y = avgCAPE)) +
 ...
 scale_x_discrete(breaks = seq(1, 12, 1), labels = month.abb) +
  ...
  coord_polar(start =0) +
  labs(x = "Month", y = "Normalized Averages of CAPE [J/kg]") +
      #ggtitle("c") +
  theme_minimal() )
```
...
```{r}
CAPE_m2 <- lmer(maxCAPE ~ SSTgradient_10day + avg_Nino3410day + NAO_10day + PNA_10day + MJO_10day + Lat + Lon + Year (1|Month) + (1|Hour_UTC), data = BigDays.sfdfT)
...
```{r}
CAPE_m8 <- lmer(maxCAPE ~ SSTgradient_5day + PNA_5day +  MJO_5day + Lon + (1|Month) + (1|Hour_UTC), data = BigDays.sfdfT)
summary(CAPE_m8)
```
...
```{r}
CAPE_model <- lmer(maxCAPE ~ SSTgradient_30day + PNA_5day +  MJO_5day + Lon + (1|Month) + (1|Hour_UTC), data = BigDays.sfdfT)
summary(CAPE_model)
```

```{r}
CAPE_m8 <- lmer(maxCAPE ~ SSTgradient_20day + PNA_5day +  MJO_10day + Lon +  (1|Month) + (1|Hour_UTC), data = BigDays.sfdfT)
summary(CAPE_m8)
```
...
newdata1 = data.frame(SSTgradient_20day = 10, PNA_5day = 3.5, MJO_10day = 4.45,  Lon = -92, Month = 4, Hour_UTC = 18)

newdata2 = data.frame(SSTgradient_20day = 22, PNA_5day = -2, MJO_10day = 4.45,  Lon = -92, Month = 4, Hour_UTC = 18)

newdata3 = data.frame(SSTgradient_20day = 40, PNA_5day = -6, MJO_10day = 4.45,  Lon = -92, Month = 4, Hour_UTC = 18)
...
CAPEpred1 <- c(10, 3.5, 2139)
CAPEpred2 <- c(22, -2, 1819)
CAPEpred3 <- c(40, -6, 1070)
test <- as.data.frame(rbind(CAPEpred1, CAPEpred2, CAPEpred3))
colnames(test) <- c("SSTgradient_20day", "PNA_5day", "value")
```

Create a prediction grid.
```{r}
library(reshape2)
pgrid <- expand.grid(SSTgradient_20day = seq(.1, 50, length = 100),
                     PNA_5day = seq(-7, 5, length = 100),
                     MJO_10day = 4.45,  
                     Lon = -92, 
                     Month = 4, 
                     Hour_UTC = 18)
...
pgridL = melt(pgrid, id.vars = c("SSTgradient_20day", "PNA_5day"), measure.vars = "pre")

#x<- matrix(pre, nrow = 5001, byrow = FALSE)
e <- c(12, 12.5, 13, 13.5)
 CAPEFIG <- ggplot(pgridL, aes(x = SSTgradient_20day, y = PNA_5day, fill = value)) +
...
#geom_contour(aes(z = value), color = "green") + 
  ...
  theme(text = element_text(size=15), legend.position="bottom", legend.key.width = unit(1.75, "cm"), 
     ...
     ggtitle("c") +
  xlab(expression(paste("20-day average SST gradient"))) + 
  ylab(expression(paste("5-day average PNA"))) +
  labs(fill = "CAPE [J / kg]") + geom_point(data = test, size = 4) 
#e <- c(12,  12.5, 13, 13.5)
...
SLBSpred1 <- predict(SLBS_model, newdata = data.frame(SSTgradient_20day = 10, NAO_20day = -0.0197, PNA_10day = 3.5,  MJO_30day = 4.45,  Lat = 37.13, Lon = -92, Year = 2020, Mo = 4), type = "response")
#7.79

SLBSpred2 <- predict(SLBS_model, newdata = data.frame(SSTgradient_20day = 22, PNA_10day = -2, NAO_20day = -0.0197, MJO_30day = 4.45,  Lat = 37.13, Lon = -92, Year = 2020, Mo = 4), allow.new.levels = TRUE, type = "response")
#18.8


SLBSpred3 <- predict(SLBS_model, newdata = data.frame(SSTgradient_20day = 40, PNA_10day = -6, NAO_20day = -0.0197, MJO_30day = 4.45,  Lat = 37.13, Lon = -92, Year = 2020, Mo = 4), allow.new.levels = TRUE, type = "response")
#32.4

SLBSpred1 <- c(10, 3.5, 8.57)
SLBSpred2 <- c(22, -2, 19.6)
SLBSpred3 <- c(40, -6, 33.2)
test <- as.data.frame(rbind(SLBSpred1, SLBSpred2, SLBSpred3))
colnames(test) <- c("SSTgradient_20day", "PNA_10day", "value")
```
...
pgridL = melt(pgrid, id.vars = c("SSTgradient_20day", "PNA_10day"), measure.vars = "pre")

#x<- matrix(pre, nrow = 5001, byrow = FALSE)
e <- c(12, 12.5, 13, 13.5)
 SLBSFIG<- ggplot(pgridL, aes(x = SSTgradient_20day, y = PNA_10day, fill = value)) +
  geom_tile( show.legend = TRUE) +
#geom_contour(aes(z = value), color = "green") + 
  #scale_fill_gradient("BuPu")
   #scale_fill_brewer(palette = "BuPu") +
  ...
  theme(text = element_text(size=15), legend.position="bottom", legend.key.width = unit(1.75, "cm"), 
     ...
     ggtitle("b") +
  xlab(expression(paste("20-day average SST gradient"))) + 
  ylab(expression(paste("10-day average PNA"))) +
  labs(fill = "SLBS [m / s]") + geom_point(data = test, size = 4) 
#e <- c(12,  12.5, 13, 13.5)
#ggplot(pgridL, aes(x = maxCAPE2, y = maxBS2, fill = value)) +
 # geom_raster(show.legend = TRUE) +
 # scale_fill_viridis_c(breaks = log(10^e), labels = c(1, 3.2, 10, 32))
...
DLBSpred1 <- predict(DLBS_model, newdata = data.frame(SSTgradient_10day = 10, NAO_20day = -0.0197, PNA_30day = 3.5,  Lat = 37.13, Lon = -92, Mo = 4), type = "response")
#7.79

DLBSpred2 <- predict(DLBS_model, newdata = data.frame(SSTgradient_10day = 22, PNA_30day = -2, NAO_20day = -0.0197, Lat = 37.13, Lon = -92, Mo = 4), allow.new.levels = TRUE, type = "response")
#18.8


DLBSpred3 <- predict(DLBS_model, newdata = data.frame(SSTgradient_10day = 40, PNA_30day = -6, NAO_20day = -0.0197, Lat = 37.13, Lon = -92, Mo = 4), allow.new.levels = TRUE, type = "response")
#32.4

DLBSpred1 <- c(10, 3.5, 19.3)
DLBSpred2 <- c(22, -2, 32.7)
DLBSpred3 <- c(40, -6, 49.9)
test <- as.data.frame(rbind(DLBSpred1, DLBSpred2, DLBSpred3))
colnames(test) <- c("SSTgradient_10day", "PNA_30day", "value")
```

Create a prediction grid. 
```{r}
library(reshape2)
pgrid <- expand.grid(SSTgradient_10day = seq(.1, 50, length = 100),
                     PNA_30day = seq(-7, 5, length = 100),
                     NAO_20day = -0.0197,
                     Lat = 37.13,
                     Lon = -92,
                     Mo = 4)
...
pgridL = melt(pgrid, id.vars = c("SSTgradient_10day", "PNA_30day"), measure.vars = "pre")

#x<- matrix(pre, nrow = 5001, byrow = FALSE)
e <- c(12, 12.5, 13, 13.5)
DLBSFIG <-  ggplot(pgridL, aes(x = SSTgradient_10day, y = PNA_30day, fill = value)) +
  geom_tile( show.legend = TRUE) +
#geom_contour(aes(z = value), color = "green") + 
  #scale_fill_gradient("BuPu")
   #scale_fill_brewer(palette = "BuPu") +
  ...
  theme(text = element_text(size=15), legend.position="bottom", legend.key.width = unit(1.75, "cm"), 
        legend.background = element_rect(size=0.5)) +
  ggtitle("a") +
  xlab(expression(paste("10-day average SST gradient"))) + 
  ylab(expression(paste("30-day average PNA"))) +
  labs(fill = "DLBS [m / s]") + geom_point(data = test, size = 4) 
#e <- c(12,  12.5, 13, 13.5)
#ggplot(pgridL, aes(x = maxCAPE2, y = maxBS2, fill = value)) +
 # geom_raster(show.legend = TRUE) +
 # scale_fill_viridis_c(breaks = log(10^e), labels = c(1, 3.2, 10, 32))
...
```{r}
ggarrange(DLBSFIG, SLBSFIG, CAPEFIG, ncol= 1, nrow=3)
```
