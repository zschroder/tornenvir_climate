...
```{r}
ClimVars2 <- data.frame(SSTgradient = BigDays.sfdfT$SSTgradient, ENSO = BigDays.sfdfT$avg_Nino3410day, NAO = BigDays.sfdfT$NAO_10day, PNA = BigDays.sfdfT$PNA_10day, MJO = BigDays.sfdfT$MJO_10day)
...
```{r}
DLBS_env <- data.frame(SSTgradient = BigDays.sfdfT$SSTgradient, ElNino = BigDays.sfdfT$avg_Nino3410day, NAO = BigDays.sfdfT$NAO_10day, PNA = BigDays.sfdfT$PNA_10day, MJO = BigDays.sfdfT$MJO_10day, DLBS = BigDays.sfdfT$maxBS_deep)
...
```{r}
DLBS_m1 <- lm(maxBS_deep ~ SSTgradient + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day, data = BigDays.sfdfT)

DLBS_m1 <- lmer(maxBS_deep ~ SSTgradient + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m1)
ranef(DLBS_m1) #There is a strong seasonal variation in DLBS values. 
```

```{r}
DLBS_m2 <- lmer(maxBS_deep ~ SSTgradient+ NAO_10day + PNA_10day+ (1|Mo), data = BigDays.sfdfT)
summary(DLBS_m2)
ranef(DLBS_m2) 
```
...
```{r}
CAPE_m1 <- lm(maxCAPE ~  SSTgradient + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day , data = BigDays.sfdfT)
hist(CAPE_m1$residuals)
sm.density(CAPE_m1$residuals, model = "Normal")

CAPE_m1 <- lmer(maxCAPE ~ SSTgradient + avg_Nino3410day + NAO_10day + PNA_10day +  MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(CAPE_m1)
ranef(CAPE_m1) #There is a strong seasonal variation in CAPE values. 
```

```{r}
CAPE_m2 <- lmer(maxCAPE ~  MJO_10day + (1|Mo), data = BigDays.sfdfT)
summary(CAPE_m2)
