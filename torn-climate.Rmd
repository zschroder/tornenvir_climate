...
```{r}
ClimVars2 <- data.frame(SSTdiff = BigDays.sfdfT$SSTgradient_15day, ENSO = BigDays.sfdfT$avg_Nino3415day, NAO = BigDays.sfdfT$NAO_15day, PNA = BigDays.sfdfT$PNA_15day, MJO = BigDays.sfdfT$MJO_15day)
...
#geom_contour(aes(z = value), color = "green") + 
  scale_fill_gradient(low = "rosybrown1", high = "firebrick4",expression(paste("CAPE [J ", kg^-1, "]"))) +
  ...
  ylab(expression(paste("15-day average SST difference"))) +
...
#geom_contour(aes(z = value), color = "green") + 
  ...
  scale_fill_gradient(low = "thistle", high = "darkorchid4",expression(paste("SLBS [m ", s^-1, "]"))) +
...
DLBSFIG <-  ggplot(pgridL, aes(x = Lon, y = SSTgradient_15day, fill = value)) +
  geom_tile( show.legend = TRUE) +
#geom_contour(aes(z = value), color = "green") + 
  scale_fill_gradient(low = "powderblue", high = "midnightblue", expression(paste("DLBS [m ", s^-1, "]"))) +
   #scale_fill_brewer(palette = "BuPu") +
  #scale_fill_viridis_c(option = "D", direction = 1, limits = c(0,45)) +
  theme_bw() +
  theme(text = element_text(size=12), legend.position="bottom", legend.key.width = unit(1, "cm"), 
